...
def seed_vaultscaler(db: Database) -> str:
    ...
    if existing:
        db.execute(
            "UPDATE clients SET internal=1, auto_publish=1, article_enabled=1, image_generation_enabled=1 WHERE id='vaultscaler'",
            commit=True,
        )
        return "vaultscaler"
    cid = db.create_client(VAULTSCALER_CLIENT, start_trial=False)
    db.execute(
        "UPDATE clients SET internal=1, auto_publish=1, article_enabled=1, image_generation_enabled=1 WHERE id=?",
        (cid,),
        commit=True,
    )
    return cid
...
def seed_ortobahn(db: Database) -> str:
    ...
    if existing:
        db.execute(
            "UPDATE clients SET internal=1, auto_publish=1, article_enabled=1, image_generation_enabled=1 WHERE id='ortobahn'",
            commit=True,
        )
        return "ortobahn"
    ...
    if default and default.get("name") == "Ortobahn":
        ...
        db.execute(
            "UPDATE clients SET internal=1, auto_publish=1, article_enabled=1, image_generation_enabled=1 WHERE id='default'",
            commit=True,
        )
        return "default"
