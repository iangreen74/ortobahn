...
"""
            ...
            if tests_passed:
                ...
                # 12. Push branch and create auto-merge PR
                pr_url = ""
                try:
                    push_branch(branch_name)
                    pr_url = create_pr(
                        branch_name=branch_name,
                        title=f"cto: {task['title']}",
                        body=(
                            f"## Autonomous Engineering\n\n"
                            f"**Task:** {task['title']}\n"
                            f"**Priority:** P{task.get('priority', 3)}\n"
                            f"**Files:** {', '.join(files_written)}\n\n"
                            f"Generated by Ortobahn CTO Agent."
                        ),
                    )
                    if pr_url:
                        enable_auto_merge(pr_url)
                        logger.info("Created auto-merge PR: %s", pr_url)
                except Exception as e:
                    logger.warning("Failed to create PR (commit is on branch %s): %s", branch_name, e)
