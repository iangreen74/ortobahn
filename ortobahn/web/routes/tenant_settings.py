...
async def tenant_toggle_auto_publish(
    ...
    # Lock graduation status when user manually toggles auto-publish
    graduation_status = "locked_auto" if enabled else "locked_manual"
    db.execute(
        "UPDATE clients SET auto_publish=?, auto_graduation_status=?, target_platforms=?,"
        " posting_interval_hours=?, platform_schedule=?, image_generation_enabled=? WHERE id=?",
        (enabled, graduation_status, target_platforms, interval, schedule_json, image_gen, client["id"]),
        commit=True,
    )
